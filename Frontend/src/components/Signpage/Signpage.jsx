...
const Signpage = () => {
  ...
  const clickHandler = async(event) => {
    ...
    const res = await fetch('http://localhost:7000/api/v1/student/studentsingin' , 
      {
        method : "POST",
        headers : {
  ...
  return (
    <div className='bg-gray-800 text-black flex justify-around items-center flex-wrap'>
      ...
      <div className=' '>
        ...
        <input onChange={signHandler} className='py-1' type="email" placeholder='Enter Email Address' name="email" required /><br/>
      </div><br/>
      <div className=''> 
        <div >
        <label className='font-bold'>Password: </label><br/>
        <input onChange={signHandler} className='py-1' type="password" placeholder='Enter Password' name="password" id="pwd" required /><br/><br/>
        </div>
